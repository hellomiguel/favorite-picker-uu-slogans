...
(function (root, factory) {
  ...
  PickerState.prototype.resetBatchSize = function () {
    ...
    this.arrays.evaluating = this.arrays.current.splice(
      0,
      this.getBatchSize(
        this.arrays.current.length + this.arrays.survived.length
      )
    );
    this.batchSize = this.arrays.evaluating.length;
  };
  ...
  PickerState.prototype.pick = function (picked) {
    ...
    for (i = 0; i < evaluating.length; i++) {
      if (
        !picked.length ||
        this.findByIdentifier(evaluating[i], picked) !== -1
      ) {
  ...
  function Picker(options) {
    ...
    for (i = 0; i < options.items.length; i++) {
      ...
      if (this.itemMap.hasOwnProperty(options.items[i].id)) {
        console.error(
          "You have more than one item with the same ID (" +
            options.items[i].id +
            ")! Please ensure the IDs of your items are unique."
        );
        return;
      }
      if (
        ...
        console.error(
          "You have defined a shortcode length of " +
            options.shortcodeLength +
            "; however, you have an item with a shortcode that does not match this length (" +
            options.items[i].shortcode +
            "). The shortcode functionality only works if the item shortcodes are of a consistent length.",
          options.items[i]
        );
        return;
      }
  ...
  Picker.prototype.getShortcodeLink = function () {
     ...
     */
    return (
      "?" + this.options.favoritesQueryParam + "=" + this.getShortcodeString()
    );
  };
  ...
  Picker.prototype.parseShortcodeString = function (shortcodeString) {
    ...
    for (i = 0; i < shortcodeString.length; i += this.options.shortcodeLength) {
      shortcode = shortcodeString.substring(
        i,
        i + this.options.shortcodeLength
      );
